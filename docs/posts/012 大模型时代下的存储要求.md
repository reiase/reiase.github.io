---
title: 从 Agentic Search 看大模型时代的存储优化
date: 2026-04-26
tags:
  - LLM
  - Agent
...
comments: true
description: >-
  本文从 Agentic Search 出发，自顶向下分析大模型时代的存储优化：Memory、Agent Harness、数据格式与工程封装共同服务于一条核心引用路径，让信息在合适时刻以合适形态、以可控成本被再次使用。
---
...
## 1. 从生成到引用：Agentic Search 的存储问题

### 1.1 推理不只是 next-token prediction
...
![Agentic Search 将全局知识库压缩成局部工作集](imgs/agentic-search/local-working-set.svg){width=100%}
...
## 2. 引用什么：Memory、Harness 与数据格式

### 2.1 Memory：历史、偏好与任务状态
...
### 2.3 数据格式：从结构化工件到 AI-native dataset
...
在轻量级和快速迭代阶段，Markdown、YAML、JSON 与 Jinja 往往承担这种建模功能。Markdown 适合承载规则、语境、边界、流程与说明，例如 `AGENTS.md`、`SKILL.md`、Dataset Card；YAML 适合表达配置、split、子集入口和模板元数据；JSON 适合承载可严格解析的样本结构、评测协议、通过标准与工具调用记录；Jinja 则把 prompt 建模成“样本到输入/输出”的映射函数。
...
## 3. 如何读出：Lance 存储结构与 Agent Search 局部性

### 3.1 Lance 如何组织 embedding、metadata 与 payload
...
### 3.3 IVF_PQ 查询中的候选召回与 payload 回查
...
![IVF+PQ 索引与 payload 回查示意](imgs/agentic-search/ivf-pq-layout-clean.svg){width=100%}

### 3.4 Agent Search 的局部性：领域、会话、租户与候选集
...
![Agentic Search 局部性对 IVF+PQ 与物理布局的影响](imgs/agentic-search/locality-impact.svg){width=100%}
...
### 3.5 对象存储路径上的 range read、IOPS 与 bridge bytes
...
## 4. 如何优化：Reorder、Adaptive nprobe 与策略封装

### 4.1 Reorder：把语义局部性转成物理局部性
...
![Reorder 与 adaptive nprobe 优化的是引用路径中的不同环节](imgs/agentic-search/reorder-adaptive-nprobe.svg){width=100%}

### 4.3 实验室验证：收益来自重引用路径，而不是所有查询
...
### 4.4 Policy 封装：让不同 query 进入不同成本曲线
...
- `Logger` 和 `Hook` 负责记录候选、注入片段、token 成本、延迟、IO 与失败原因。
...
![基于 OPQ/PQ 的渐进式索引设想](imgs/agentic-search/progressive-opq-index.svg){width=100%}
...
- [LanceDB and binary data lake](https://ericfu.me/lancedb-and-binary-data-lake/)
