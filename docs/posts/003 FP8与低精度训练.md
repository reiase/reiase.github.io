...
自 Nvidia 2022 年发布 H100 GPU 集成 FP8 支持以来，该格式作为首个面向训练场景设计的 8 位浮点规范，曾被寄望延续 TF32 在推理加速和 BF16 在训练稳定性上的成功，延续 Huang's Law。硬件层面 2 倍于 FP16 的理论算力提升，使其被视为突破大模型算力墙的关键路径：不仅能减少计算和存储的开销，还能大幅提升训练速度。但是，古尔丹，代价是什么呢？

FP8 端到端的性能提升只有 30%，距离理论值 100%相去甚远；使用 FP8 需要面临精度风险，很多实验发现不小于 2%的 loss diff；需要修改训练框架，精巧的延迟缩放策略，需要框架层面引入除了同步、异步以外的第三种时序；需要在代码里增加两种全新的数值格式，E4M3 和 E5M2；需要付出额外的显存，TransformerEngine 官方文档提到，显存开销需要增加 5%。代价就是这一切。
...
DeepSeek 团队已经分析了影响精度的主要原因之一——矩阵规模。此处我们更加系统性的讨论影响精度的因素有哪些。算子精度涉及各种不同的数据类型、矩阵形状和运算类型(attention，matmul等)，导致精度分析问题变得极为复杂。为了简化精度分析工作，我们通过对模型中的各种运算进行分析，发现大多数计算可以拆解成向量内积操作的集合。因此，通过研究向量内积操作的精度，即可对算子精度进行一个系统全面的分析。以LLM模型为例，涉及到的主要运算如下：
...
我们讨论了TransformerEngine与DeepSeek V3使用的两种不同的FP8训练方案，并且从向量内积的量化噪声分析入手，对比了两者在随机向量与相关向量下的表现。在完全随机向量的情况下，通过一步步引入优化，逐步提高内积计算的信噪比。无法确定DeepSeek团队在设计自己的FP8方案时，是否使用了类似的量化分析手段。但这种分析方法也许可以帮助我们进一步改进FP8或者其他低精度方案。

当分析到相关向量的内积信噪比时，我认为出现了所谓的"Aha Moment"。对比TransformerEngine，DeepSeek的方案虽有所优势，但还不够惊艳。但考虑向量相关性后，我们观察到了非常明显的信噪比提升。我认为这种提升并非是无心的结果，而是经过可以设计的。毕竟在真实任务上验证完整的FP8方案需要巨额的投入，很难想象DeepSeek团队在模型上直接端到端训练来进行优化手段的消融实验。
